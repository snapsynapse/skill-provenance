...
description: >
  Version tracking for skills and their associated files across Claude sessions
  and surfaces (Chat, Cowork, Code). Embeds version identity inside files rather
  than filenames. Maintains a manifest and changelog that travel with the skill
  bundle. Use this skill whenever opening, saving, or handing off a skill project
...
Skill projects move between Claude sessions (Chat, Cowork, Code) and between
local storage (Obsidian, working directories, git repos). Version identity gets
lost when it lives only in filenames. A file renamed from `SKILL_v4.md` to
`SKILL_v5.md` with no internal record of what changed creates ambiguity that
costs real time to resolve.
